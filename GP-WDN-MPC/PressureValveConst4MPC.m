function [constraints,c_estimate_value,ValveFlagOut] = PressureValveConst4MPC(X,Pressure_Valves_Matrix_Index,X0,base,NodeElevations,ValveSetting,ValveTypesString,verify,ValveStatus,ValveFlag,Solution,FlowUnits,iter,IndexInVar)
    % variable =
    % [ head of Junction;
    %   head of reservior;
    %   head of tank;
    ...
    openness_valve_start_index = min(IndexInVar.ValveOpennessIndex);
    openness_valve_end_index = max(IndexInVar.ValveOpennessIndex);
    
    
    Q = X(q_valve_start_index:q_valve_end_index);
    O = X(openness_valve_start_index:openness_valve_end_index);
    H = X(h_start_index:h_end_index);
    c_estimate_value = C_Estimate_Valve(X0,base,q_valve_start_index,q_valve_end_index);
