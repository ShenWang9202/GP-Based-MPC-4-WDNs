function constraints = PressurePumpConst4MPC(X,Pressure_Pump_Matrix_Index,X0,base,verify,PumpStatus,IndexInVar)
    % variable =
    % [ head of Junction;
    %   head of reservior;
    %   head of tank;
...
%                  H(DeliveryIndex) * H(SuctionIndex)^(-1) * base^(-PumpEquation(i,1)) <= 1;
              ]; % h_d <= h_s + HeadIncreasing

            monomial_temp = 1.0;
            monomial_temp = monomial_temp * H(SuctionIndex) * H(DeliveryIndex)^(-1) ;
            ...
            f_mono = [f_mono;monomial_temp * (Q(i))^(r_vector(i)*q_pump(i)^(w_vector(i)-1)*s(i)^(2-w_vector(i))) * S(i)^(a_s(i))];
            %f_mono = [f_mono;monomial_temp * (Q(i))^(PumpEquation(i,2)*q_pump(i)^(PumpEquation(i,3)-1)) * base^(PumpEquation(i,1) * s(i)^2)];
            %f_mono = [f_mono;monomial_temp * (Q(i))^(PumpEquation(i,2)*q_pump(i)^(PumpEquation(i,3)-1)) * c_s(i) * S(i)^(d_s(i))] ;
            %         else
            %             % flow == 0 if it is close
            %             constraints = [constraints; Q(i) == base^(0);];
        end
    end
    [m,~] = size(f_mono);
    if(m>0)
